# Hi there 👋 I'm Nikita Maksimov

Frontend Developer with over **four years of commercial experience**.  
I build **high-performance, scalable web applications** using **React**, **TypeScript**, and **Next.js**.

My focus is frontend **architecture**, **performance**, and **code quality**.  
I work with complex interfaces, migrate legacy codebases, and turn heavy business logic into clear, maintainable UI.

I’m especially interested in dashboards, real-time apps, and projects where frontend is more than “just UI”.

🌍 Open to collaboration on interesting projects, side initiatives, and open-source contributions.

---

## 🚀 Technical Skills

- **Languages:** JavaScript, TypeScript  
- **Markup:** HTML, CSS (Sass, Less, SCSS)  
- **Frameworks & Libraries:** React, Next.js, MobX, Redux, React Native  
- **Tools:** Vite, Webpack, Git, Storybook, Rollup  

<details>
<summary>📊 <strong>GitHub Stats</strong></summary>

![Nikita's GitHub stats](https://github-readme-stats.vercel.app/api?username=JohnImril&show_icons=true&theme=outrun)

</details>

<details>
<summary>📈 <strong>Top Languages</strong></summary>

[![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=JohnImril&layout=compact&theme=outrun)](https://github.com/anuraghazra/github-readme-stats)

</details>

---

## 💬 **Let's Connect!**

- **Email:** [john.maks595@gmail.com](mailto:john.maks595@gmail.com)  
- **Telegram:** [@JohnImril](https://t.me/JohnImril)  
- **LinkedIn:** [Nikita Maksimov](https://www.linkedin.com/in/john-imril/)
